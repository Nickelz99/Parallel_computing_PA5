...
__global__ void minimum_distance(float * X, float * Y, volatile float * D, int n) {
    ...
    if(i < n)
    {
        ...
        // if (D[0] > l_min)
        // {
        //     printf("Thread_id = %i distance = %f \n", i,l_min);
        //     // D[0] = l_min;
        //     atomicExch(D,l_min);
        //     __threadfence_block();
        // }
        int* address_as_i = (int*) D;
        int assumed;
        int old = *address_as_i;
        do {
            assumed = old;
            old = ::atomicCAS(address_as_i, assumed,
                __float_as_int(::fminf(l_min, __int_as_float(assumed))));
        } while (assumed != old);
...
int main(int argc, char* argv[]) {
    ...
    if (threads_per_block < MAX_BLOCK_SIZE + 1)
    {
        minimum_distance<<<blocks,threads_per_block>>>(dVx, dVy, dmin_dist, num_points);
    }
    else // if there are more points then threads per block allowed
    {
        rem = num_points/MAX_BLOCK_SIZE;
        blocks = rem + 1;
